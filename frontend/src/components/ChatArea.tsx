...
function normalizeDeliverableSection(content: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    while (cursor + 2 < lines.length) {
      ...
      if (
        /^(##|###|第\d+步|根据|总结|核心|技术栈|系统架构|关键约束|如果需)/.test(
          row[0],
        )
      ) {
        break;
...
function normalizeStepSections(content: string): string {
  ...
  normalized = normalized.replace(
    /(^|\n)(实施路线图（简化版）|实施路径|实施步骤|步骤规划)\s*[:：]?\s*/g,
    "$1## $2\n\n",
  );
  normalized = normalized.replace(/(^|\n)(阶段|任务|优先级)\s*(?=\n|$)/g, "$1");
  normalized = normalized.replace(
  ...
  normalized = normalized.replace(
    /(^|\n)(MVP 核心功能验证|模块架构深化|接口与前端交互设计|UI 与交互优化|测试与迭代优化)\s*[:：]?\s*/g,
    "$1- $2：",
  );
  normalized = normalized.replace(
    /(^|\n)([^\n：]{2,20})\s*[：:]\s*(☆☆☆|★★★|高|中|低)\s*(?=\n|$)/g,
...
function rebuildCompressedMermaid(lines: string[]): string[] {
  ...
  let rebuilt = source
    ...
    .replace(
      /(flowchart\s+(?:TD|LR)|graph\s+(?:TD|LR)|sequenceDiagram|classDiagram|stateDiagram|erDiagram|journey|gantt|pie|mindmap|timeline)/g,
      "\n$1\n",
    )
    .replace(/%%\s*/g, "\n%% ")
    ...
    .replace(
      /(;)(?=\s*(?:classDef|style|subgraph|end|[A-Za-z0-9_]+\[|[A-Za-z0-9_]+\{|[A-Za-z0-9_]+\(|[A-Za-z0-9_]+-->))/g,
      "$1\n",
    )
    .replace(
      /((?:-->|==>|-.->)\s*[A-Za-z0-9_]+(?:\[[^\]]*\]|\([^\)]*\)|\{[^\}]*\}))(?!\s*(?:classDef|style|subgraph|end|%%|$))/g,
      "$1\n",
    )
    .replace(
    ...
    .replace(
      /(end)(?=\s*(?:[A-Za-z0-9_]+\[|[A-Za-z0-9_]+\{|[A-Za-z0-9_]+\(|subgraph|classDef|style|%%))/g,
      "$1\n",
    )
    .replace(/(\w+)(-->|==>|-.->)(\w+)/g, "$1 $2 $3")
...
function normalizeMermaidSection(content: string): string {
  ...
  const flushMermaidBuffer = () => {
    ...
    const sanitizedLines = rebuiltLines.filter((line) =>
...
function normalizeSummarySections(content: string): string {
  ...
  normalized = normalized.replace(
    /([^\n])(是否将|建议采用|建议优先|需要确认|优先实现)/g,
    "$1\n\n$2",
  );
  normalized = normalized.replace(
    /([^\n])(产品定位|解决的核心问题|实施路径|技术选型建议|后续演进方向|用户价值主张|目标用户群体|技术架构分层)\s*-/g,
    "$1\n$2 -",
  );
...
function normalizeVisualSeparators(content: string): string {
  ...
  normalized = normalized.replace(
    /(^|\n)\s*(结论|答案|统计依据|计算过程|关键数据|补充说明|注意事项|下一步)\s*[：:]\s*/g,
    "$1### $2\n\n",
  );
  normalized = normalized.replace(
...
function parseSuggestionSegments(
  ...
  const matches = [
    ...cleaned.matchAll(
      /(类别|分类|类型|方向|主题|模块|方案|说明|概述|结论|判断|特点|核心观点|建议|行动建议|推荐|做法|策略|适用场景|适用情况|适用对象|适用阶段|适配场景|优点|优势|收益|好处|缺点|风险|不足|限制|注意事项)\s*[：:]/g,
    ),
  ];
  if (matches.length < 2) {
    return [];
  ...
  for (let index = 0; index < matches.length; index += 1) {
    ...
    const end =
      index + 1 < matches.length ? matches[index + 1].index! : cleaned.length;
    const value = cleaned
      .slice(start, end)
      .trim()
      .replace(/^[，、；;]+|[，、；;]+$/g, "");
    ...
    while (cursor < lines.length) {
      ...
      blockRows.push(
        rowSegments
          .map(
            (segment) =>
              `${suggestionFieldLabels[segment.key] ?? segment.label}：${segment.value}`,
          )
          .join(" | "),
      );
      cursor += 1;
...
function normalizeInlineMarkdownTables(content: string): string {
  ...
  const normalized = lines.map((line) => {
    ...
    for (let index = 0; index < tokens.length; index += 1) {
      ...
      for (
        ;
        end < tokens.length && isMarkdownSeparatorCell(tokens[end]);
        end += 1
      ) {
    ...
    if (
      ...
      !headers.every(
        (cell) => looksLikePseudoTableHeader(cell) || cell.length <= 20,
      )
    ) {
      return line;
    }
  ...
  const normalized = lines.map((line) => {
    ...
    if (rowSegments.length >= 2 && rowSegments[0].length >= 2) {
      ...
      const hasExplicitSeparator =
        separatorLike.length === headers.length &&
        separatorLike.every((cell) => isMarkdownSeparatorCell(cell));
      const bodyRows = (
        hasExplicitSeparator ? rowSegments.slice(2) : rowSegments.slice(1)
      ).filter((segment) => segment.length === headers.length);
      const remainderSegments = hasExplicitSeparator
        ? rowSegments.slice(2 + bodyRows.length)
        : rowSegments.slice(1 + bodyRows.length);

      if (
        bodyRows.length > 0 &&
        headers.every(
          (cell) => looksLikePseudoTableHeader(cell) || cell.length <= 24,
        )
      ) {
        const rebuilt = [
          `| ${headers.join(" | ")} |`,
        ...
        if (finalNote) {
          rebuilt.push("");
          rebuilt.push(
            finalNote.startsWith(">") ? finalNote : `> ${finalNote}`,
          );
        }
        return `\n${rebuilt.join("\n")}\n`;
      }
    ...
    if (containsDenseTableSeparator(tableSource)) {
      ...
      for (
        ...
        isMarkdownSeparatorCell(parts[separatorEnd]);
        separatorEnd += 1
      ) {
    ...
    const looksLikePairwiseTable =
      ...
      parts.every((cell, index) =>
        index % 2 === 0
          ? looksLikePseudoTableHeader(cell) || cell.length <= 16
          : cell.length > 0,
      );
...
function normalizeFragmentedPipeTableBlocks(content: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      (!currentHasPipe && !nextHasPipe) ||
      shouldStopFragmentedTableBlock(current)
    ) {
      normalized.push(lines[index]);
      continue;
    }
    ...
    while (cursor < lines.length) {
      ...
      if (
        cursor < lines.length &&
        shouldStopFragmentedTableBlock(lines[cursor])
      ) {
        break;
      }
    }
    ...
    if (
      expectedCols < 2 ||
      expectedCols > 5 ||
      flattened.length < expectedCols * 2
    ) {
      normalized.push(lines[index]);
      continue;
    }
    ...
    for (
      let start = 0;
      start + expectedCols <= flattened.length;
      start += expectedCols
    ) {
...
function normalizeVerticalKeyValueTableBlocks(content: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      !headers.every((cell) => looksLikePseudoTableHeader(cell)) ||
      !isLikelyTableValueLine(firstRowSecondCell)
    ) {
      normalized.push(lines[index]);
      continue;
    }
...
function parseStructuredSpreadsheetSummary(
  line: string,
): StructuredSpreadsheetBlock | null {
...
function parseStructuredSpreadsheetRow(
  line: string,
  headers: string[],
): string[] | null {
...
function buildStructuredSpreadsheetMarkdown(
  block: StructuredSpreadsheetBlock,
): string {
  const lines: string[] = [];
  lines.push(
    `<div class="md-spreadsheet-note">数据来源：${block.fileName}${block.sheetName ? ` / ${block.sheetName}` : ""}</div>`,
  );
  lines.push("");
...
`,
  ...
  fixed = fixed.replace(
    /([^\n])\s+(总结|结论|建议|风险|下一步|关键任务|阶段功能目标|理由|备注|关键依赖)[:：]/g,
    "$1\n\n$2：",
  );
    ...
    const renderChart = async () => {
      try {
        ...
        const hasSvgContent = Boolean(
          renderedSvg && renderedSvg.includes("<svg"),
        );
        const hasSyntaxError =
          /Syntax error in text|Parse error|Lexical error/i.test(renderedSvg);
...
const ChatArea: React.FC<ChatAreaProps> = ({
  ...
  const [kbPickerAnchor, setKbPickerAnchor] = useState<{
    top: number;
    left: number;
    minWidth: number;
  } | null>(null);
  ...
  const kbPillButtonRef = useRef<HTMLButtonElement | null>(null);
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event: MouseEvent) => {
      if (
        kbPickerRef.current &&
        !kbPickerRef.current.contains(event.target as Node)
      ) {
  ...
  const handleToggleKbPicker = () => {
    setIsKbPickerOpen((prev) => {
      const next = !prev;
      if (next && kbPillButtonRef.current) {
        const rect = kbPillButtonRef.current.getBoundingClientRect();
        setKbPickerAnchor({
          top: rect.bottom + 6,
          left: rect.left,
          minWidth: rect.width,
  ...
  const secondaryToolbarItems = [
    {
      kind: chatMode === "think" ? "think" : "fast",
  ...
  const handleKbPickerSelect = (knowledgeBaseId: string) => {
    setIsKbPickerOpen(false);
    onChangeKnowledgeBase(knowledgeBaseId);
  };
  ...
  return (
    <main
      className={`chat-area ${sidebarOpen ? "sidebar-open" : "sidebar-closed"}`}
    >
      <div className="chat-topbar">
        <div className="chat-topbar-left">
          ...
          <span className="chat-topbar-sep">·</span>
          <span className="chat-topbar-hint">
            {formatTime(activeConversation.updatedAt)}
          </span>
        </div>

        <div className="chat-topbar-pills">
          <div className="topbar-pill-kb-wrap" ref={kbPickerRef}>
            <button
              ref={kbPillButtonRef}
              type="button"
              className={`topbar-pill topbar-pill-kb${isKbPickerOpen ? " topbar-pill-kb--open" : ""}`}
              title={scopeText}
              aria-haspopup="listbox"
              aria-expanded={isKbPickerOpen}
              onClick={handleToggleKbPicker}
            >
              <span className="topbar-pill-text">{scopeText}</span>
              <span className="topbar-pill-caret">▾</span>
            </button>
            {isKbPickerOpen && kbPickerAnchor && (
              <div
                className="kb-picker-dropdown"
                role="listbox"
                style={{
                  position: "fixed",
                  top: kbPickerAnchor.top,
                  left: kbPickerAnchor.left,
                  minWidth: Math.max(kbPickerAnchor.minWidth, 220),
                }}
              >
                <button
                  type="button"
                  className={`kb-picker-option${
                    !selectedKnowledgeBase ? " kb-picker-option--active" : ""
                  }`}
                  onClick={() => handleKbPickerSelect("")}
                >
                  <span className="kb-picker-option-icon">✕</span>
                  <span className="kb-picker-option-label">无知识库</span>
                  {!selectedKnowledgeBase && (
                    <span className="kb-picker-option-check">✓</span>
                  )}
                </button>
                {knowledgeBases.length === 0 ? (
                  <div className="kb-picker-empty">暂无知识库</div>
                ) : (
                  knowledgeBases.map((kb) => {
                    const isActive = selectedKnowledgeBase?.id === kb.id;
                    return (
                      <button
                        key={kb.id}
                        type="button"
                        className={`kb-picker-option${
                          isActive ? " kb-picker-option--active" : ""
                        }`}
                        onClick={() => handleKbPickerSelect(kb.id)}
                      >
                        <span
                          className="kb-picker-option-label"
                          title={kb.name}
                        >
                          {kb.name}
                        </span>
          ...
          {secondaryToolbarItems.map((item) => (
            <div
              key={item.text}
              className={`topbar-pill topbar-pill--${item.kind}`}
              title={item.text}
            >
      ...
      <div className="messages-container">
        {activeConversation.messages.length === 0 ? (
          <WelcomeHome
            ...
            hasKnowledgeBase={Boolean(selectedKnowledgeBase)}
            onSelectConversation={onSelectConversation}
            onSelectKnowledgeBase={onChangeKnowledgeBase}
            onSendPrompt={(text) => {
            ...
            return (
              <div key={message.id} className={`message ${message.role}`}>
                ...
                >
                  {degradedMetadata && (
                    <div
                      className="message-degraded-banner"
                      role="status"
                      aria-live="polite"
                    >
                    ...
                    <ReactMarkdown
                      ...
                      components={{
                        code({ className, children, ...props }) {
                          ...
                          if (
                            !isInline &&
                            className?.includes("language-advice-cards")
                          ) {
                        ...
                        td({ children, ...props }) {
                          const text = React.Children.toArray(children)
                            ...
                            .trim();
                          const highlight =
                            /(推荐|优先|适合|高|强烈建议|最佳实践|风险|注意)/.test(
                              text,
                            );
                          return (
                            <td
      ...
      <div className="prompt-list">
        {(selectedKnowledgeBase ? suggestedPrompts : generalPrompts).map(
          (prompt) => (
            <button
              key={prompt}
              type="button"
