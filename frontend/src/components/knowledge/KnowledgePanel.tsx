...
const KnowledgePanel: React.FC<KnowledgePanelProps> = ({
  ...
  const statusLabel = (status: string) => {
    if (status === "indexed")
      return { text: "已索引", color: "#16a34a", bg: "#dcfce7" };
    if (status === "processing")
      return { text: "处理中", color: "#d97706", bg: "#fef3c7" };
  ...
  const uploadProgressPercent =
    directoryUploadTask.eligibleFiles > 0
      ? Math.round(
          (directoryUploadTask.processedFiles /
            directoryUploadTask.eligibleFiles) *
            100,
        )
      : 0;

  const visibleFailedItems = useMemo(
    () => directoryUploadTask.failedItems,
    [directoryUploadTask.failedItems],
  );
            ...
            <div className="kb-header-left">
              <div>
                ...
                <p className="kb-header-sub">
                  ...
                  {knowledgeBases.reduce((s, kb) => s + kb.documents.length, 0)}{" "}
                  份文档
                </p>
              </div>
            </div>
              ...
              <div className="kb-list">
                {knowledgeBases.map((kb) => {
                  ...
                  return (
                    <div
                      key={kb.id}
                      className={`kb-card${isSelected ? " kb-card--active" : ""}`}
                    >
                      {/* 知识库卡片头部 */}
                      <div className="kb-card-header">
                        ...
                        >
                          <div className="kb-card-info">
                            ...
                            {kb.description && (
                              <span className="kb-card-desc">
                                {kb.description}
                              </span>
                            )}
                            <span className="kb-card-meta">
                              {kb.documents.length} 份文档 · 创建于{" "}
                        ...
                        <div className="kb-card-actions">
                          ...
                          <label
                            className="kb-upload-btn kb-upload-btn--secondary"
                            title="上传目录"
                          >
                            上传目录
                            <input
                              ref={(element) =>
                                registerDirectoryInput(kb.id, element)
                              }
                              type="file"
                              multiple
                              className="hidden-input"
                      ...
                      {isSelected &&
                        isTaskVisible &&
                        directoryUploadTask.knowledgeBaseId === kb.id && (
                          <div className="kb-upload-task-shell">
                            <div className="kb-upload-task-compact">
                              <div className="kb-upload-task-compact-main">
                                <span
                                  className={`kb-upload-task-pill kb-upload-task-pill--${directoryUploadTask.status}`}
                                >
                                  {directoryUploadTask.status === "scanning" &&
                                    "扫描中"}
                                  {directoryUploadTask.status === "uploading" &&
                                  ...
                                  {directoryUploadTask.status === "canceled" &&
                                    "已取消"}
                                  {directoryUploadTask.status === "done" &&
                                    "已完成"}
                                  {directoryUploadTask.status ===
                                    "partial-failed" && "部分完成"}
                                  {directoryUploadTask.status === "failed" &&
                                    "失败"}
                                </span>
                                <div className="kb-upload-task-compact-text">
                                  <div className="kb-upload-task-compact-title">
                                    目录上传任务
                                  </div>
                                  <div className="kb-upload-task-compact-summary">
                                    {directoryUploadTask.processedFiles}/
                                    {directoryUploadTask.eligibleFiles} · 成功{" "}
                                    {directoryUploadTask.successFiles} · 失败{" "}
                                    {directoryUploadTask.failedFiles} · 跳过{" "}
                                    {directoryUploadTask.skippedFiles}
                                  </div>
                                </div>
                              </div>
                              <div className="kb-upload-task-actions">
                                <button
                                  ...
                                  onClick={() =>
                                    setShowUploadTaskDetails((prev) => !prev)
                                  }
                                >
                                ...
                                {canContinueUpload && (
                                  <button
                                    className="kb-upload-task-btn"
                                    onClick={onContinueDirectoryUpload}
                                  >
                                    继续上传
                                  </button>
                                )}
                            ...
                            {showUploadTaskDetails && (
                              <div className="kb-upload-task">
                                <div className="kb-upload-progress-meta">
                                  <span>
                                    已处理 {directoryUploadTask.processedFiles}{" "}
                                    / {directoryUploadTask.eligibleFiles}
                                  </span>
                                ...
                                <div className="kb-upload-stats-grid">
                                  <div className="kb-upload-stat-card">
                                    <span className="kb-upload-stat-label">
                                      总文件
                                    </span>
                                    <strong>
                                      {directoryUploadTask.totalFiles}
                                    </strong>
                                  </div>
                                  <div className="kb-upload-stat-card">
                                    <span className="kb-upload-stat-label">
                                  ...
                                  <div className="kb-upload-stat-card">
                                    ...
                                    <strong>
                                      {directoryUploadTask.successFiles}
                                    </strong>
                                  </div>
                                  <div className="kb-upload-stat-card">
                                    <span className="kb-upload-stat-label">
                                    ...
                                    <strong>
                                      {directoryUploadTask.failedFiles}
                                    </strong>
                                  </div>
                                  <div className="kb-upload-stat-card">
                                    <span className="kb-upload-stat-label">
                                  ...
                                  <div className="kb-upload-stat-card">
                                    ...
                                    <strong>
                                      {directoryUploadTask.pendingFiles}
                                    </strong>
                                  </div>
                                </div>
                                ...
                                {showFailedItems &&
                                  visibleFailedItems.length > 0 && (
                                    <div className="kb-upload-issues">
                                      <div className="kb-upload-issues-title">
                                        失败文件
                                      </div>
                                      {visibleFailedItems.map((item) => (
                                        <div
                                          key={`${item.path}-${item.reason}`}
                                          className="kb-upload-issue-item"
                                        >
                                          <span className="kb-upload-issue-path">
                                            {item.path}
                                          </span>
                                          <span className="kb-upload-issue-reason">
                                            {item.reason}
                                          </span>
                                        </div>
                                      ))}
                                ...
                                {directoryUploadTask.skippedItems.length >
                                  0 && (
                                  <div className="kb-upload-issues-toggle-row">
                                    <button
                                      className="kb-upload-task-btn kb-upload-task-btn--ghost"
                                      onClick={() =>
                                        setShowSkippedItems((prev) => !prev)
                                      }
                                    >
                                      {showSkippedItems
                                        ? "隐藏已跳过文件"
