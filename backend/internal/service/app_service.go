...
func NewAppService(qdrant *QdrantService, store *AppStateStore, chatHistory ChatHistoryStore, serverConfig model.ServerConfig, userStore *UserStore) *AppService {
	service := &AppService{
	...
	if store != nil {
		if loadedState, err := store.Load(); err != nil {
			...
			// One-shot migration: move legacy state.json userConfigs into SQLite,
			// then drop the field on the next saveState.
			if userStore != nil && len(loadedState.UserConfigs) > 0 {
				migrated := 0
				for uid, cfg := range loadedState.UserConfigs {
					if err := userStore.UpsertUserConfig(uid, cfg.Chat, cfg.Embedding); err != nil {
						log.Printf("migrate user config uid=%s: %v", uid, err)
						continue
					}
					migrated++
				}
				log.Printf("migrated %d user config(s) from state.json to SQLite", migrated)
			}
		}
	}
...
func (s *AppService) saveState() error {
	...
	state := persistentAppState{
		...
		// UserConfigs intentionally omitted — per-user config now lives in SQLite (user_configs table).
	}
	s.state.Mu.RUnlock()
...
// GetConfig returns chat/embedding loaded from the SQLite user_configs table for the given user
// (falling back to system defaults when the row does not exist), merged with the global MCP block.
// userID == "" → returns the global default (used by system / mcp paths).
func (s *AppService) GetConfig(userID string) model.AppConfig {
	s.state.Mu.RLock()
	cfg := s.state.Config
	s.state.Mu.RUnlock()

	if userID != "" && s.userStore != nil {
		if chat, emb, ok, err := s.userStore.GetUserConfig(userID); err != nil {
			log.Printf("get user config uid=%s: %v", userID, err)
		} else if ok {
			cfg.Chat = chat
			cfg.Embedding = emb
		}
...
func (s *AppService) RegisterStagedUpload(userID, uploadID, knowledgeBaseID, fileName string) (model.Document, error) {
	if s == nil || s.staging == nil {
	...
	resolvedKnowledgeBaseID, err := s.ResolveKnowledgeBaseID(userID, knowledgeBaseID)
	if err != nil {
		return model.Document{}, err
	}
	...
	uploaded, err := s.IndexDocument(userID, document)
	if err != nil {
		return model.Document{}, err
	}
...
func (s *AppService) UpdateConfig(userID string, req model.ConfigUpdateRequest) (model.AppConfig, error) {
	if strings.TrimSpace(userID) == "" {
		return model.AppConfig{}, fmt.Errorf("user id is required")
	}
	if s.userStore == nil {
		return model.AppConfig{}, fmt.Errorf("user store is not configured")
	}
	...
	chat := model.ChatConfig{
		...
		ExtraHeaders:        strings.TrimSpace(req.Chat.ExtraHeaders),
	}
	...
	if s.hasSensitiveStructuredDocuments() && !IsLocalOllamaConfig(chat, emb) {
		return model.AppConfig{}, fmt.Errorf("sensitive structured documents require local ollama for both chat and embedding")
	}

	if err := s.userStore.UpsertUserConfig(userID, chat, emb); err != nil {
		return model.AppConfig{}, err
	}

	s.state.Mu.RLock()
	globalMCP := s.state.Config.MCP
	s.state.Mu.RUnlock()

	merged := model.AppConfig{Chat: chat, Embedding: emb, MCP: globalMCP}
	merged.MCP.BasePath = defaultMCPBasePath(merged.MCP.BasePath)
	if strings.TrimSpace(merged.MCP.Token) == "" {
		merged.MCP.Token = generateMCPToken()
	}
	return merged, nil
}
...
func (s *AppService) ListKnowledgeBases(userID string) []model.KnowledgeBase {
	s.state.Mu.RLock()
	knowledgeBases := make([]model.KnowledgeBase, 0, len(s.state.KnowledgeBases))
	for _, kb := range s.state.KnowledgeBases {
		if userID != "" && kb.UserID != userID {
			continue
		}
...
func (s *AppService) CreateKnowledgeBase(userID string, req model.KnowledgeBaseInput) (model.KnowledgeBase, error) {
...
func (s *AppService) DeleteKnowledgeBase(userID, id string) (int, error) {
	s.state.Mu.Lock()
...
func (s *AppService) GetKnowledgeBaseDocuments(userID, id string) ([]model.Document, error) {
	s.state.Mu.RLock()
	defer s.state.Mu.RUnlock()
...
func (s *AppService) ResolveKnowledgeBaseID(userID, candidate string) (string, error) {
	s.state.Mu.RLock()
	defer s.state.Mu.RUnlock()

	candidate = strings.TrimSpace(candidate)
	if candidate != "" {
		kb, ok := s.state.KnowledgeBases[candidate]
		if !ok {
	...
	for id, kb := range s.state.KnowledgeBases {
		if userID == "" || kb.UserID == userID {
			return id, nil
		}
	}
...
func (s *AppService) IndexDocument(userID string, document model.Document) (model.Document, error) {
	...
	// 用 userID 解析 embedding 配置：用户在 Settings 改过 embedding URL/key 会读到
	// 之前用 currentEmbeddingConfig()（空 userID）只读系统默认，导致用户配置失效
	vectors, err := s.rag.EmbedTexts(context.Background(), s.currentEmbeddingConfigFor(userID), chunkTexts(chunks), s.qdrantVectorSize())
	if err != nil {
		return model.Document{}, err
	}
...
func (s *AppService) DeleteDocument(userID, knowledgeBaseID, documentID string) (model.Document, error) {
	s.state.Mu.Lock()
...
func (s *AppService) BuildRetrievalContext(userID string, req model.ChatCompletionRequest) (string, []map[string]string, error) {
	startedAt := time.Now()
	chunks, err := s.EvaluateRetrieve(userID, req)
	if err != nil {
		return "", nil, err
	}
...
func (s *AppService) EvaluateRetrieve(userID string, req model.ChatCompletionRequest) ([]RetrievedChunk, error) {
	if s == nil {
	...
	// F3: chat with no KB binding → pure chat, skip retrieval entirely.
	// Empty KnowledgeBaseID + empty DocumentID = explicit "no RAG" intent
	// (the legacy fallback would have used "all user KBs", which leaks data
	// across the conversation-level binding the frontend now enforces).
	if strings.TrimSpace(req.KnowledgeBaseID) == "" && strings.TrimSpace(req.DocumentID) == "" {
		logRetrievalStageMetrics(req, query, "retrieval_skipped_no_kb_binding", startedAt, map[string]any{
			"status": "ok",
			"reason": "conversation is not bound to any knowledge base",
		})
		return nil, nil
	}

	// Short-circuit: if the user has no accessible KB for this request, skip the
	// (expensive, often misconfigured) query embedding entirely. Pure chat works
	// without an embedding endpoint set up.
	if s.qdrant == nil || !s.qdrant.IsEnabled() {
		return nil, nil
	}
	if kbIDs, err := s.resolveRetrievalKnowledgeBaseIDs(userID, req); err == nil && len(kbIDs) == 0 {
		logRetrievalStageMetrics(req, query, "retrieval_skipped_no_kb", startedAt, map[string]any{
			"status": "ok",
			"reason": "user has no accessible knowledge base for this request",
		})
		return nil, nil
	}
	...
	if s.queryRewriter == nil {
		...
		defer cancel()
		vectors, err := s.rag.EmbedTexts(embedCtx, s.resolveEmbeddingConfig(userID, req), []string{query}, s.qdrantVectorSize())
	...
	chunks, err := s.retrieveRelevantChunks(userID, req, queryVector)
...
func (s *AppService) CurrentChatConfig(userID string) model.ChatModelConfig {
	if s == nil {
		return model.ChatModelConfig{}
	}
	return s.currentChatConfigFor(userID)
}

func (s *AppService) BuildChatContext(userID string, req model.ChatCompletionRequest) (string, []map[string]string, error) {
	s.state.Mu.RLock()
	defer s.state.Mu.RUnlock()
	...
	// Filter to user-owned KBs only.
	ownedKBs := make([]model.KnowledgeBase, 0, len(s.state.KnowledgeBases))
	for _, kb := range s.state.KnowledgeBases {
		if userID == "" || kb.UserID == userID {
			ownedKBs = append(ownedKBs, kb)
		}
	}

	if len(ownedKBs) == 0 {
		return "当前用户尚未创建知识库。", nil, nil
	}

	kbNames := make([]string, 0, len(ownedKBs))
	for _, kb := range ownedKBs {
		kbNames = append(kbNames, kb.Name)
	}
	sort.Strings(kbNames)

	return "当前未限定知识库范围，系统将默认使用当前用户的全部知识库作为后续检索候选。当前知识库包括：" + strings.Join(kbNames, "、"), nil, nil
}
...
func (s *AppService) currentEmbeddingConfig() model.EmbeddingModelConfig {
	return s.currentEmbeddingConfigFor("")
}

// currentEmbeddingConfigFor returns the embedding config for the given user, falling back to
// the global default when userID is empty or the user has no saved config.
func (s *AppService) currentEmbeddingConfigFor(userID string) model.EmbeddingModelConfig {
	if userID != "" && s.userStore != nil {
		if _, emb, ok, err := s.userStore.GetUserConfig(userID); err == nil && ok &&
			strings.TrimSpace(emb.Provider) != "" && strings.TrimSpace(emb.BaseURL) != "" && strings.TrimSpace(emb.Model) != "" {
			return model.EmbeddingModelConfig{
				Provider: strings.TrimSpace(emb.Provider),
...
func (s *AppService) currentChatConfig() model.ChatModelConfig {
	return s.currentChatConfigFor("")
}

// currentChatConfigFor returns the chat config for the given user, falling back to
// the global default when userID is empty or the user has no saved config.
func (s *AppService) currentChatConfigFor(userID string) model.ChatModelConfig {
	if userID != "" && s.userStore != nil {
		if chat, _, ok, err := s.userStore.GetUserConfig(userID); err == nil && ok &&
			strings.TrimSpace(chat.Provider) != "" && strings.TrimSpace(chat.BaseURL) != "" && strings.TrimSpace(chat.Model) != "" {
			limit := chat.ContextMessageLimit
			if limit <= 0 {
				limit = 12
			}
			return model.ChatModelConfig{
				Provider:            strings.TrimSpace(chat.Provider),
				BaseURL:             strings.TrimSpace(chat.BaseURL),
				Model:               strings.TrimSpace(chat.Model),
				APIKey:              strings.TrimSpace(chat.APIKey),
				Temperature:         chat.Temperature,
				ContextMessageLimit: limit,
				ExtraHeaders:        chat.ExtraHeaders,
			}
		}
	}
	s.state.Mu.RLock()
...
func (s *AppService) resolveEmbeddingConfig(userID string, req model.ChatCompletionRequest) model.EmbeddingModelConfig {
	cfg := req.Embedding
	...
	return s.currentEmbeddingConfigFor(userID)
}

func (s *AppService) resolveChatConfig(userID string, req model.ChatCompletionRequest) model.ChatModelConfig {
	cfg := req.Config
	cfg.Provider = strings.TrimSpace(cfg.Provider)
	cfg.BaseURL = strings.TrimSpace(cfg.BaseURL)
	cfg.Model = strings.TrimSpace(cfg.Model)
	...
	if cfg.Provider != "" && cfg.BaseURL != "" && cfg.Model != "" {
		if cfg.ContextMessageLimit <= 0 {
			cfg.ContextMessageLimit = s.currentChatConfigFor(userID).ContextMessageLimit
		}
		return cfg
	}
	return s.currentChatConfigFor(userID)
...
func (s *AppService) ContextMessageLimit(userID string) int {
	if s == nil {
		return 12
	}
	limit := s.currentChatConfigFor(userID).ContextMessageLimit
	if limit <= 0 {
		return 12
	}
	return limit
...
func (s *AppService) SaveConversation(userID string, req model.SaveConversationRequest) (*model.Conversation, error) {
	if s == nil {
		return nil, fmt.Errorf("app service is nil")
	}
	...
	if err := s.chatHistory.SaveConversation(userID, conversation); err != nil {
		return nil, err
	}
	return &conversation, nil
}

func (s *AppService) ListConversations(userID string) ([]model.ConversationListItem, error) {
	if s == nil {
		return nil, fmt.Errorf("app service is nil")
	}
	if s.chatHistory == nil {
	...
	items, err := s.chatHistory.ListConversations(userID)
	if err != nil {
		return nil, err
	}
...
func (s *AppService) GetConversation(userID, id string) (*model.Conversation, error) {
	if s == nil {
		return nil, fmt.Errorf("app service is nil")
	}
	if s.chatHistory == nil {
	...
	return s.chatHistory.GetConversation(userID, id)
}

func (s *AppService) DeleteConversation(userID, id string) error {
	if s == nil {
	...
	return s.chatHistory.DeleteConversation(userID, id)
}
...
func (s *AppService) retrieveRelevantChunks(userID string, req model.ChatCompletionRequest, queryVector []float64) ([]RetrievedChunk, error) {
	if s.qdrant == nil || !s.qdrant.IsEnabled() {
		return nil, nil
	}

	knowledgeBaseIDs, err := s.resolveRetrievalKnowledgeBaseIDs(userID, req)
	if err != nil {
		return nil, err
	}
	...
	if s.queryRewriter != nil {
		if setter, ok := s.queryRewriter.(interface {
			...
			setter.SetChatConfigProvider(func() model.ChatModelConfig {
				return s.resolveChatConfig(userID, req)
			})
		}
		...
		embeddingConfig := s.resolveEmbeddingConfig(userID, req)
...
func (s *AppService) resolveRetrievalKnowledgeBaseIDs(userID string, req model.ChatCompletionRequest) ([]string, error) {
	s.state.Mu.RLock()
	defer s.state.Mu.RUnlock()
