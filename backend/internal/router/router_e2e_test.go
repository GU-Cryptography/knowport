...
func newTestRouter(t *testing.T) (*http.ServeMux, string, func()) {
	...
	appService := service.NewAppService(qdrantService, service.NewAppStateStore(""), chatHistoryStore, serverConfig, nil)
	_, err = appService.UpdateConfig("test-user", model.ConfigUpdateRequest{
	...
	appHandler := handler.NewAppHandler(serverConfig, appService, service.NewLLMService(), toolPlanner, nil, service.NewModelProbeService())
	mcpServer := mcp.NewServer(mcpRegistry, appService, serverConfig)
	ginEngine := NewRouter(appHandler, serverConfig, mcpServer, nil)
