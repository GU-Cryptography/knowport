...
type AppHandler struct {
	...
	modelProbeService *service.ModelProbeService
}

func NewAppHandler(serverConfig model.ServerConfig, appService *service.AppService, llmService *service.LLMService, toolPlanner *mcp.ToolUsePlanner, userStore *service.UserStore, modelProbeService *service.ModelProbeService) *AppHandler {
	return &AppHandler{
		...
		userStore:         userStore,
		modelProbeService: modelProbeService,
	}
}
...
func (h *AppHandler) GetConfig(c *gin.Context) {
	uid := auth.UserIDFrom(c)
	c.JSON(http.StatusOK, h.appService.GetConfig(uid))
}
...
func (h *AppHandler) ProbeModels(c *gin.Context) {
	var req struct {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		writeError(c, http.StatusBadRequest, "invalid probe request body")
		return
	}
	if strings.TrimSpace(req.APIURL) == "" {
		writeError(c, http.StatusBadRequest, "api_url is required")
		return
	}
	if req.Kind != "chat" && req.Kind != "embedding" {
		req.Kind = "chat"
	}
	result, err := h.modelProbeService.Probe(c.Request.Context(), service.ProbeRequest{
		APIURL: req.APIURL,
		APIKey: req.APIKey,
		Kind:   req.Kind,
	})
...
func (h *AppHandler) ListConversations(c *gin.Context) {
	uid := auth.UserIDFrom(c)
	items, err := h.appService.ListConversations(uid)
	if err != nil {
		writeError(c, http.StatusInternalServerError, err.Error())
		return
	}
...
func (h *AppHandler) GetConversation(c *gin.Context) {
	uid := auth.UserIDFrom(c)
	conversation, err := h.appService.GetConversation(uid, c.Param("id"))
	if err != nil {
		writeError(c, http.StatusInternalServerError, err.Error())
		return
	}
...
func (h *AppHandler) SaveConversation(c *gin.Context) {
	...
	conversation, err := h.appService.SaveConversation(uid, req)
	if err != nil {
...
func (h *AppHandler) DeleteConversation(c *gin.Context) {
	uid := auth.UserIDFrom(c)
	if err := h.appService.DeleteConversation(uid, c.Param("id")); err != nil {
		writeError(c, http.StatusInternalServerError, err.Error())
		return
	}
...
func (h *AppHandler) UpdateConfig(c *gin.Context) {
	...
	cfg, err := h.appService.UpdateConfig(uid, req)
	if err != nil {
		writeError(c, http.StatusBadRequest, err.Error())
		return
	}
...
func (h *AppHandler) ListKnowledgeBases(c *gin.Context) {
	uid := auth.UserIDFrom(c)
	c.JSON(http.StatusOK, gin.H{"items": h.appService.ListKnowledgeBases(uid)})
}
...
func (h *AppHandler) CreateKnowledgeBase(c *gin.Context) {
	...
	knowledgeBase, err := h.appService.CreateKnowledgeBase(uid, req)
	if err != nil {
		writeError(c, http.StatusBadRequest, err.Error())
		return
	}
...
func (h *AppHandler) DeleteKnowledgeBase(c *gin.Context) {
	uid := auth.UserIDFrom(c)
	remaining, err := h.appService.DeleteKnowledgeBase(uid, c.Param("id"))
	if err != nil {
...
func (h *AppHandler) ListDocuments(c *gin.Context) {
	uid := auth.UserIDFrom(c)
	items, err := h.appService.GetKnowledgeBaseDocuments(uid, c.Param("id"))
	if err != nil {
		writeError(c, http.StatusNotFound, err.Error())
		return
	}
...
func (h *AppHandler) StageUpload(c *gin.Context) {
	...
	if err := validateUploadFile(file, h.appService.GetConfig(uid)); err != nil {
		writeError(c, http.StatusBadRequest, err.Error())
		return
	}
...
func (h *AppHandler) DeleteDocument(c *gin.Context) {
	uid := auth.UserIDFrom(c)
	removedDocument, err := h.appService.DeleteDocument(uid, c.Param("id"), c.Param("documentId"))
	if err != nil {
		writeError(c, http.StatusNotFound, err.Error())
		return
	}
...
func (h *AppHandler) ChatCompletions(c *gin.Context) {
	...
	preparedReq, sources, err := h.prepareChatRequest(c.Request.Context(), uid, req)
	if err != nil {
		writeError(c, http.StatusBadRequest, err.Error())
		return
	}
	...
	if assistantMessage := firstAssistantChoice(response); assistantMessage != nil {
		_, saveErr := h.appService.SaveConversation(uid, model.SaveConversationRequest{
...
func (h *AppHandler) prepareChatRequest(ctx context.Context, userID string, req model.ChatCompletionRequest) (model.ChatCompletionRequest, []map[string]string, error) {
	if len(req.Messages) == 0 {
		return model.ChatCompletionRequest{}, nil, fmt.Errorf("messages cannot be empty")
	}

	retrievalContext, retrievalSources, err := h.appService.BuildRetrievalContext(userID, req)
	if err != nil {
	...
	contextSummary, sources, err := h.appService.BuildChatContext(userID, req)
	if err != nil {
		return model.ChatCompletionRequest{}, nil, err
	}
	...
	preparedReq := req
	preparedReq.Config = h.appService.CurrentChatConfig(userID)
	preparedReq.Config.ContextMessageLimit = h.appService.ContextMessageLimit(userID)
...
func (h *AppHandler) handleUpload(c *gin.Context, candidateKnowledgeBaseID string) {
	...
	knowledgeBaseID, err := h.appService.ResolveKnowledgeBaseID(uid, candidateKnowledgeBaseID)
	if err != nil {
		writeError(c, http.StatusBadRequest, err.Error())
		return
	}
	...
	uploaded, err := h.appService.IndexDocument(uid, document)
	if err != nil {
