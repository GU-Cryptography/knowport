...
type AppServiceReader interface {
	GetConfig(userID string) model.AppConfig
	ListKnowledgeBases(userID string) []model.KnowledgeBase
	GetKnowledgeBaseDocuments(userID, id string) ([]model.Document, error)
	...
	BuildRetrievalContext(userID string, req model.ChatCompletionRequest) (string, []map[string]string, error)
	CreateKnowledgeBase(userID string, req model.KnowledgeBaseInput) (model.KnowledgeBase, error)
	DeleteKnowledgeBase(userID, id string) (int, error)
	DeleteDocument(userID, knowledgeBaseID, documentID string) (model.Document, error)
	SaveConversation(userID string, req model.SaveConversationRequest) (*model.Conversation, error)
	DeleteConversation(userID, id string) error
	ResolveKnowledgeBaseID(userID, candidate string) (string, error)
	IndexDocument(userID string, document model.Document) (model.Document, error)
	StageInlineUpload(fileName string, content []byte, source string) (model.StagedUpload, error)
	RegisterStagedUpload(userID, uploadID, knowledgeBaseID, fileName string) (model.Document, error)
}
