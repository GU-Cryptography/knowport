...
仓库根目录有一份 `env.example` 模板。复制后编辑成你自己的 `.env`（`.env` 已 gitignore，不会进版本控制）：

```bash
cp env.example .env
```

然后用编辑器打开 `.env`，**至少**改两项必填的密钥（模板里标了 `CHANGE_ME` 的就是必改项）：

```bash
# 生成 JWT 签名密钥（48 字节随机 base64）
echo "JWT_SECRET=$(openssl rand -base64 48)"

# 生成 API Key 加密密钥（32 字节随机 base64）
echo "CONFIG_ENCRYPTION_KEY=$(openssl rand -base64 32)"
```

把两条 echo 的输出贴回 `.env` 里替换对应的 `CHANGE_ME` 行。

**关键提示**：

- `JWT_SECRET` 决定登录态签名安全性，必须随机；改了之后所有现有用户会被踢下线。
- `CONFIG_ENCRYPTION_KEY` 用来加密用户在前端设置面板填的模型 API Key。**一旦设置后丢了，所有用户存的 Key 都解不出来**——务必备份到密码管理器或其他物理介质。
- `QDRANT_VECTOR_SIZE` 必须与你后续在前端设置面板里选的 embedding 模型一致：bge-m3 → 1024，nomic-embed-text → 768。Qdrant collection 第一次创建后维度就固定了，改维度需要清掉 qdrant 数据卷。

`.env` 在 `docker compose up` 时被 compose CLI 自动加载（要求 `.env` 与 `docker-compose.yml` **同级**），里面的变量替换 compose 文件里 `${VAR:-...}` 占位符，再通过 `environment:` 段注入容器进程。
